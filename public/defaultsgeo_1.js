...
function getListFromServer() {
	...
	/*var data=localStorage.getItem("comp_data4");
		if(data==null){
		$.ajax({
		type: "GET",
		dataType:'json',
...
for(var n=0;n<data.camps_poly.length;n++){
		var polycrd=[];
		var string = data.camps_poly[n].coordinates;
		var a = string.split(",");
		for (var i = 0; i < a.length; i++) {
		var a2 = a[i].split(":");
		//var aa={lng:a2[1],lat:a2[0]};
		var ll=new google.maps.LatLng(a2[0], a2[1]);
		polycrd.push(ll);
		}

		//alert(polycrd.toString());
		...
		strokeOpacity: 1.0,
		strokeWeight: .5,
		fillColor: data.camps_poly[n].color,
		fillOpacity: 0.4,
		title:data.camps_poly[n].label,
        data:data.camps_poly[n],
		camp_id:data.camps_poly[n].camp_id,
		label:data.camps_poly[n].label
		});
		...
		//camps_info.setContent(JSON.stringify(mapsMouseEvent.latLng.toJSON(), null, 2));
		...
		console.log(lnk);
		//camps_info.setPosition(mapsMouseEvent.latLng);
		//camps_info.open(map);

		});
		
		//google.maps.event.addListener("click", (mapsMouseEvent) => {
