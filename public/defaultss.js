...
Loader.prototype.addAllFilterList1=addAllFilterList;
loder.load().then(() => {
...
function getListFromServer() {
	...
	var data=null;//localStorage.getItem("comp_data4");
		...
		type: "GET",
		dataType:'json',
		//url: "http://8.213.22.228/api/getCompanyData",//'avl_comp':'الأجهزة المحمولة'
...
//var search_c_list='';//'أفاقي';
//var search_tc_list='';//'مجموعة';
function addAllFilterList() {
	var search_c_list = $('input[name="searchcomp_list"]').val();
	var search_tc_list = $('input[name="searchtcomp_list"]').val();
    $('#companylist').empty();
    var len = company_list.length;

    for (var n = 0; n < len; n++) {
		if(search_c_list=='' || company_list_ar[n].indexOf(search_c_list)>=0){
        var tr = $('<tr/>');
    ...
    for (var n = 0; n < len; n++) {
		if(search_tc_list=='' || trans_comp_list_ar[n].indexOf(search_tc_list)>=0){
        tr = $('<tr/>');
...
function updateDeviceDetails(data){
	...
	'<div style="width:180px;text-align:right;padding-left:10px">'+getTransCompany(data.m.s)+'</div>';//changed m.t to m.s
   ...
   if( devln!=0){
    document.getElementById("avlcomp").innerHTML=getCompanyName(data.m.x);
...
function getVehicles_data(map) {
			...
			//url: "https://tracking.naqaba.com.sa/api/getDevicesData2",
			url: "/api/getDevicesData2",
