...
const LOADER_OPTIONS = {
    apiKey: "AIzaSyAN0ZcCiG3WT9NUgfJXgFLebI4s2QEXzJE",
    version: "weekly",
    libraries: [],
};
...
var trees=[];
const mapOptions = {
    center: { lat:21.42384457242216 , lng: 39.830496643104745 },
    zoom: 13,
	mapTypeId: 'roadmap',
	zoomControl: false,
    scaleControl: false,
...
Loader.prototype.filter_geofence1=viewPolygon;
loder.load().then(() => {
...
function getListFromServer() {
	...
	$.ajax({
		type: "GET",
		dataType:'json',
		url: "/api/getAllGeoCodes",
		//url: "https://tracking.naqaba.com.sa/geocode.txt",
		//url: "https://tracking.naqaba.com.sa/api/getAllGeoCodes",
		data:{},	cache: false,
		success: function(data)
		{
			//var x=JSON.stringify(data);
			//var len=x.length;
			//var l1=parseInt(len/50);
			//var x1=x.substr(0,l1);
			//localStorage.setItem("geodata",x1);
			//console.log(JSON.stringify(data).length);//7022839
		...
		//ajax close
	//loadlocalgeofencePolygon();
	
	//localStorage.removeItem("comp_data1");
	//localStorage.removeItem("comp_data4");
	var data=null;//localStorage.getItem("comp_data4");
		if(data==null){
		$.ajax({
		type: "GET",
		dataType:'json',
		//url: "https://tracking.naqaba.com.sa/api/getCompanyData",
		url: "/api/getCompanyData",
		data:{},	cache: false,
		success: function(data)
		{
			parseData(data);
			//localStorage.setItem("comp_data4",JSON.stringify(data));
			getVehicles_data(map);
		},
		error:function(jqXHR, textStatus, errorThrown) {
		}
...
function loadlocalgeofencePolygon(){
...
function viewPolygon(data){
for(var n=0;n<camp_polys.length;n++){
	camp_polys[n].setMap(null);
}
	
var ids = selectedValues1($('input[name="season_id"]'));


for(var n=0;n<data.cpolys.length;n++){
	if(isvalidData1(data.cpolys[n].Season,ids)){
	
		var polycrd=[];
		for (var i = 0; i < data.cpolys[n].coordinates[0].length; i++) {
		var ll=new google.maps.LatLng(data.cpolys[n].coordinates[0][i][1],data.cpolys[n].coordinates[0][i][0]);
		polycrd.push(ll);
		}
		...
		fillColor: '#ff0000',
		fillOpacity: 0.4,
		title:data.cpolys[n].label,
		label:data.cpolys[n].label
		});
		
		camp_poly.setMap(map);
		camp_polys.push(camp_poly);
		
	/*	google.maps.event.addListener(camp_poly, 'click', (mapsMouseEvent)=> {
	    var lat=mapsMouseEvent.latLng.lat();
		var lng=mapsMouseEvent.latLng.lng();
		var lnk="https://tracking.naqaba.com.sa/api/getGeoFenceInfo?lng="+lng+"&lat="+lat+"&token=cebc8011932a85c60a7e079b840bf083161812d3";
		console.log(lnk);
		});	
		
		
		var camps_info1 = new google.maps.InfoWindow({
    content: ''
});

        google.maps.event.addListener(camp_poly, 'mouseover', function (e) {
         var x='</br></br>'+this.label+'</br></br>';
        camps_info1.setContent(x);
        camps_info1.setPosition(e.latLng);
        camps_info1.open(map);
        });

        google.maps.event.addListener(camp_poly, 'mouseout', function (e) {
        camps_info1.close();
        });*/
...
var dev_comp_list = ["BCE", "Teltonika", "Machinestalk", "Ruptela", "Queclink","Afaqi 2"];
var dev_comp_list_value = ["3000", "3001", "3002", "3003", "3004","3006"];
var imei_list = [];
...
var season_list = ["رمضان","المنافذ","المشاعر المقدسة","صلوات الحج"];


function addAllFilterList() {
	
	 $('#seasonlist').empty();
    var len = season_list.length;

    for (var n = 0; n < len; n++) {
        var tr = $('<tr/>');
        tr.append('<td><input type="checkbox" name="season_id" id="season_id" value="' + season_list[n] + '" class="cbox2" onchange="updateFilterList2();"><br></td>');
        tr.append('<td style="color:#000000;"  class="textStyle">' + season_list[n] + '</td>');
        $("#seasonlist").append(tr);
    }
...
function updateDeviceDetails(data){
	...
	row+='<div style="width:390px;float:right;text-align:right;padding-right:10px;margin-top:10px;font-weight:bold">GeoFence:</div>';
	
	row+='<div style="width:390px;float:right;text-align:right;padding-right:10px;">'+data.l+'</div>';
...
function getVehicles_data(map) {
			...
			type: "GET",
			dataType:'json',
			url: "/api/getGeofenceDevice",
			//url: "https://tracking.naqaba.com.sa/api/getGeofenceDevice",
...
return m.toLocaleString('en-US', { timeZone: 'Asia/Riyadh' });
}
