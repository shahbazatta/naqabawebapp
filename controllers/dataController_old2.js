...
exports.getListBusesGeofenceTimePeriod = async (req, res) => {
  ...
  let start_dt = req.query.start_dt;
  let start_tm = req.query.start_tm;
  let end_dt = req.query.end_dt;
  let end_tm = req.query.end_tm;
	...
	filters['data.pt'] = geocode;
	   ...
	   let data = dbCon.gpsLiveDevices.find(filters).project( hideOutputFields );
				   ...
				   if(trecords.length>0){
					...
					records=trecords;
					let json1='{"status":'+200+',"result":"success","data":'+JSON.stringify(records)+'}';
					res.send(json1);
					return;
