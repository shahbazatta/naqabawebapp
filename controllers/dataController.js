      ...
      if (!st_dt || !nd_dt || st_dt < 0 || nd_dt < 0) {
         ...
         if (imeis.length > 0) {
            await imeis.forEach(im => {
               ...
               let rec = calculateTripCount_4(im, records1, src_geo, dest_geo);
               if (rec != null)
...
function calculateTripCount_4(im, records, src_geo, dest_geo) {
   let a = [];
   if (records.length > 1) {
      let pos = 1;
      ...
      records.forEach(r => {
         ...
         if (i == 3) {
			 tmtaken+=(etm_tmp-stm_tmp)
            let j = {startTime: stm_tmp, endTime: etm_tmp};
            a.push(j);
            stm_tmp = r[3];
            cnt++;
            i = 1;
      ...
      if (cnt > 0) {
		  let avgtime=parseInt(tmtaken/cnt/1000);
         let json1 = {imei: im, AverageRoundTripTime: avgtime, src_code: src_geo, dest_code: dest_geo, data: a};
         return json1;
      }
   }
};
...
function calculateTripCount_3(im, records, src_geo, dest_geo) {
   ...
   if (records.length > 1) {
      ...
      if (cnt1 > 0 || cnt2 > 0) {
         ...
         let json1 = {
            imei: im,
            AverageTripTimeSrc_to_Dest: avgtm_s_to_d,
            AverageTripTimeDest_to_Src: avgtm_d_to_s,
            data: a
         };
         return json1;
      }
...
//API Spatial3
/** return Round trip data for all IMEIs (buses)
 * @param req
 * @param res
 * @returns {Promise<void>}
 */
exports.getTTforAllBussesTimePeriodT2 = async (req, res) => {
      ...
      if (!st_dt || !nd_dt || st_dt < 0 || nd_dt < 0) {
         ...
         if (imeis.length > 0) {
            await imeis.forEach(im => {
               ...
               let rec = calculateTripCount_5(im, records1, src_geo, dest_geo);
               if (rec != null)
                  records_all.push(rec);
            });
         }
...
function calculateTripCount_5(im, records, src_geo, dest_geo) {
   let a = [];
   if (records.length > 1) {
      let pos = 1;
      records.sort((a, b) => a[pos].localeCompare(b[pos]));
      ...
      if (cnt > 0) {
		  let tot_tm=parseInt((etm_tmp-stm_tmp)/1000);
         let json1 = {imei: im, TotalTimeTaken: tot_tm, src_code: src_geo, dest_code: dest_geo, RoundStartTime:stm_tmp,RoundEndTime:etm_tmp};
         return json1;
      }
   }
...
exports.getTTforAllBussesTimePeriod1 = async (req, res) => {
   // let imei = req.query.imei;
   let src_geo = req.query.src_geo;
   let dest_geo = req.query.dest_geo;
   let sdt = req.query.start_dt;
      ...
      if (!st_dt || !nd_dt || st_dt < 0 || nd_dt < 0) {
         ...
         if (imeis.length > 0) {
            await imeis.forEach(im => {
               ...
               let rec = calculateTripCount_11(im, records1, src_geo, dest_geo);
               if (rec != null)
                  records_all.push(rec);
            });
         }
...
function calculateTripCount_11(im, records, src_geo, dest_geo) {
   let a = [];
   if (records.length > 1) {
      let pos = 1;
      records.sort((a, b) => a[pos].localeCompare(b[pos]));
      ...
      records.forEach(r => {
         ...
         if (i == 3) {
			 let m1 = new Date(parseInt(stm_tmp));
            let dt1 = m1.toLocaleString();
			
			let m2 = new Date(parseInt(etm_tmp));
           let dt2 = m2.toLocaleString();
		   //,startTime_standard: dt1, endTime_standard: dt2
			
            let j = {startTime: stm_tmp, endTime: etm_tmp,startTime_standard: dt1, endTime_standard: dt2};
            a.push(j);
            stm_tmp = r[3];
            cnt++;
            i = 1;
...
function calculateTripCount_1(im, records, src_geo, dest_geo) {
   ...
   if (records.length > 1) {
      ...
      if (cnt > 0) {
         let json1 = {imei: im, ToalRoundTrips: cnt, src_code: src_geo, dest_code: dest_geo, data: a};
         return json1;
      }
   }
...
exports.getAllImeis = async (req, res) => {
   ...
   let records = await dbCon.gpsLive.find(filter).project({_id: 0, 'data.imei': 1, 'data.odata': 1}).toArray();
   if (records.length > 0) {
...
/**
 * Return all IMEIS which are not in AVL devices
 * @param req
 * @param res
 * @returns {Promise<void>}
 */
exports.getStatusUnknown = async (req, res) => {
   let status = 400;
   let result = 'error';
   let msg = '';
   let filter = {'device.imei': {$exists: false}};
   let data = await dbCon.gpsLiveDevices.find(filter).project({_id: 0, 'data.imei': 1,'data.odata': 1});
   let records = [];
   await data.forEach(r => {
	  let json1 = {imei: r.data.imei, port: r.data.odata};
      records.push(json1);
   });
   if (records.length > 0) {
      res.send({status: 200, result: "success", count: records.length, data: records});
      return;
   } else {
      status = 404;
      msg = 'No record';
   ...
   if (len > 0) {
      ...
      records.forEach(r => {
         ...
         if (i == 3) {
            ...
            etm.push(etm_tmp);
            stm.push(stm_tmp);
            diff.push(parseInt((etm_tmp - stm_tmp) / 1000));
      ...
      if (diff.length > 0) {
         let json1 = {imei: im, TimeIn: 'seconds', TotalTimeTaken: diff, RoundStartTime: stm, RoundEndTime: etm};
         return json1;
      }
   }
...
exports.getGeofenceDevice = async (req, res) => {
   ...
   let currentTime = Date.now();
   let oldTime = currentTime - 86400000;
   filters['data.updatedon'] = {$gte: oldTime, $lte: currentTime};
...
exports.getImeisData2 = async (req, res) => {
   let imei = req.query.imei;
   let busid = req.query.busid;
   let avlComp = req.query.avl_comp;
   let transComp = req.query.trans_comp;
...
exports.getAllGeoCodes = async (req, res) => {
  
	  let filters = {};
      let data = dbCon.geoFences.find(filters);
      let records = [];
      await data.forEach(r => {
		  let info = {};
            info.coordinates = r.geometry.coordinates;
			info.label = r.attributes.Code_ID;
			info.Season=r.attributes.Season;
            records.push(info);
 
      });
      if (records.length > 0) {
		 let json = {result: "success", cpolys: records};
         res.send(json);
         return;
      } else {
         status = 404;
         msg = 'no records found';
		 let json = {status: status, msg: msg};
         res.send(json);
      }
