...
// API 33
/**
 * Get All IMEIs which are sending data at inconsistent time period for a given date time
 * @param req
 * @param res
            ...
            let data = await dbCon.gpsHistory.aggregate([
               ...
               },
               {
                  $match: {
                     $expr: {
                        $gte: [
                           {$divide: [{$subtract: ["$prev", "$avltm"]}, 60000]},
                           minits
                        ]
                     }
                  }
...
function calculateTripCount(im, records, src_geo, dest_geo) {
   ...
   let json = {imei: im, trip_cnt: 0, trip_cnt_src_to_dest: 0, trip_cnt_dest_to_src: 0, data: records};
   return json;
}
