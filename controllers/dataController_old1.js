...
exports.getBusStatus = async (req, res) => {
  ...
  let accuracy = req.query.accuracy;
	...
	if (!lat || isNaN(lat)) {
	 //return res.error({error: 'Invalid latitude value'});
	  return res.send('Invalid latitude value');
	}
	
	if (!lng || isNaN(lng)) {
	 //return res.error({error: 'Invalid longitude value'});
	 return res.send('Invalid longitude value');
	}
	
	if (!accuracy || isNaN(accuracy)) {
	// return res.error({error: 'Invalid Accuracy distance value'});
	return res.send('Invalid Accuracy distance value');
	}
  ...
  try {
   ...
   //filters['data.updatedon'] = {$gt: oldTime, $lte: currentTime};
   ...
   let records = [];
   await data.forEach(r => {
	  let dis= distanceInM(r.data.location.coordinates[1],r.data.location.coordinates[0],lat,lng);
	  if(dis<=accuracy){
      let info = r.data;
   ...
   // console.log(data)
   if(records.length==0){
      res.send({status:404,message:'No records found'});
      return;
   }
   res.send(records);
};
